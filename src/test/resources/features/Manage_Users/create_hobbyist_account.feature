...
@all_faa @create_hobbyist_account @create_hobbyist_account.feature
Feature: create_hobbyist_account.feature
User Stories 
...
Scenario: Create a new hobbyist account from the FAA UAS Drone Homepage
...
Scenario: Access the gmail to verify the new hobbyist account
...
Scenario: Access the FAA UAS Drone Homepage with new hobbyist account
Given i have accessed the welcome page
When i click on the Login button
Then i log on using new email account
Then i click on the Hobbyist Model option
Then i complete a hobbyist profile
Then i validate the physical and mailing address
Then i complete payment for the hobbyist drone

Scenario: Access gmail to delete all messages in your inbox
Given i have accessed the gmail login page
When i log on using newly created email address
Then i enter my gmail password 'Dummy$11'
Then i click on the gmail inbox
