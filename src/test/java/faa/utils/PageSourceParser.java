...
/**
 * Helper class that helps with parsing through table elements,
 * only need to send the css selector that matches all of the elements
 * @author jfranklin
 */
public class PageSourceParser 
{
	...
	/**
	 * Parses through page source that matches the specified css selector and pulls text/values from them 
	 * that match the given CSS Selector
	 * and stores the list in this object
	 * @param cssSelectorForElements - String - matches ALL elements in table. ie: td tags
	 * @param driver - WebDriver - ie: getDriver()
	 */
...
//	}
	...
	/**
	 * Get all the visible text from the objects that match the given CSS Selector
	 * @return
	 */
	...
	public String getRandomValue(){
		populateValuesFromAllElements();
		assertThat(valuesFromAllElements.size()).overridingErrorMessage("There were not enough values matching the CSS selector: "
				+ "CSS Selector:"
				+getCssSelector()).isGreaterThan(0);
	...
	public String getRandomText(){
		assertThat(textFromAllElements.size()).overridingErrorMessage("There were not enough values matching the CSS selector: CSS Selector:"+getCssSelector()).isGreaterThan(0);
	...
	/**
	 * if the source has changed since the last time it was loaded, you may need to refresh
	 * @param driver
	 */
